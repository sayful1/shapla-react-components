...
const FileUploader: FC<Props> = ({
    ...
    const addFiles = (files: FileList) => {
        for (let i = 0; i < files.length; i++) {
            ...
            if (chunking) {
                utils.uploadChunk(utils.generateFileObject(file), 0, args)
                    .then((response:any) => {
                ...
                utils.upload(utils.generateFileObject(file), args)
                    .then((response:any) => {
                        onSuccess && onSuccess(response.data);
                    })
                    .catch((response:any) => {
    ...
    return (
        <div className="shapla-file-uploader-container">
            <div className={`shapla-file-uploader ${isDraggedOver ? 'shapla-file-uploader--dragged' : ''}`}
                 onDragOver={(e) => {
                     e.preventDefault()
            ...
            >
                ...
                <div className="shapla-file-uploader-message">
                    ...
                    <div className="shapla-file-uploader-message__drag">{textLineOne}</div>
                    <div
                        className="shapla-file-uploader-message__browse">{textLineTwo}</div>
                    <label
                        htmlFor={inputId}
                        className="shapla-file-uploader-message__select-files shapla-button is-primary"
                    >{textButton}</label>
                    <div className="shapla-file-uploader-message__maxsize">{textMaxUploadLimit}</div>
                </div>
                {
                ...
                <div className="shapla-file-uploader-files">
                    {
                        files.map((file, index) => {
                            return <FileUploadStatus key={index} file={file} onCancel={()=>onCancelUpload(file) }/>
                        })
