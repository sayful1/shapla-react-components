...
const Select: FC<Props> = ({
  ...
  const handleKeydownEvent = (event: React.KeyboardEvent) => {
    ...
    if ("Enter" === event.code) {
      ...
      setShowDropdown(false);
      (
        (element.current as HTMLElement).querySelector(
          "input"
        ) as HTMLInputElement
      ).blur();
    }
    ...
    >
      <div className="shapla-select-field__control">
        <InputField
          ...
          rightIcon={
            <>
              {clearable && (hasSelectedOption || hasValue) && (
                <span className="icon is-right icon--delete">
        ...
        >
          <>
            {filteredOptions.map((option) => {
              return (
                <span
                  key={option.value}
                  role="option"
                  className={`shapla-dropdown-item ${dropdownItemClasses(
                    option
                  )}`}
            ...
            {!filteredOptions.length && (
              <span className="shapla-dropdown-item is-link no-options">
                <>{noOptionsText}</>
              </span>
            )}
          </>
      ...
      {hasError && (
        <small className="shapla-text-field__help-text is-invalid">
          {validationText}
        </small>
      )}

      {helpText.length ? (
        <small className="shapla-text-field__help-text"> {helpText}</small>
      ) : null}
    </div>
  );
};
