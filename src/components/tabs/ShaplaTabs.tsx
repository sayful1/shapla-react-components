...
const Shaplatab = ({
  ...
  const navItemClass = (
    tab: ReactElement<TabsDataInterface>,
    index: number
  ) => {
    const classes = [];
  ...
  useEffect(() => {
    setTabsClasses(
      ["shapla-tabs", vertical && "shapla-tabs--vertical"]
        .filter(Boolean)
        .join(" ")
    );
  }, [vertical]);
  ...
  useEffect(() => {
    const newTabs = React.Children.toArray(children).filter((child) => {
      if (React.isValidElement(child) && child.type === ShaplaTab) return child;
    });
